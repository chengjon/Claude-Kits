...
要在计划模式下开始新会话，请使用 `--permission-mode plan` 标志：
...
您也可以使用 `-p` 直接在计划模式下运行查询（即在["无头模式"](/zh-CN/docs/claude-code/sdk/sdk-headless)中）：
...
查看[设置文档](/zh-CN/docs/claude-code/settings#available-settings)获取更多配置选项。
...
<Steps>
  ...
  <Step title="引用 MCP 资源">
    ...
    这使用格式 @server:resource 从连接的 MCP 服务器获取数据。查看 [MCP 资源](/zh-CN/docs/claude-code/mcp#use-mcp-resources)获取详情。
  </Step>
</Steps>
...
<Note>
  [扩展思考](/zh-CN/docs/build-with-claude/extended-thinking)在 Claude Code 中默认禁用。您可以通过使用 `Tab` 切换思考开启来按需启用它，或者使用"think"或"think hard"等提示。您也可以通过在设置中设置 [`MAX_THINKING_TOKENS` 环境变量](/zh-CN/docs/claude-code/settings#environment-variables)来永久启用它。
</Note>

<Steps>
  <Step title="提供上下文并请 Claude 思考">
    ```
...
<Tip>
  ...
  [扩展思考](/zh-CN/docs/build-with-claude/extended-thinking)对于复杂任务最有价值，例如：
  ...
  * 评估不同方法之间的权衡
  ...
  在会话期间使用 `Tab` 切换思考的开启和关闭。

  您提示思考的方式会导致不同深度的思考：

  * "think"触发基本扩展思考
  * 强化短语如"keep hard"、"think more"、"think a lot"或"think longer"触发更深层的思考

  有关更多扩展思考提示技巧，请参阅[扩展思考技巧](/zh-CN/docs/build-with-claude/prompt-engineering/extended-thinking-tips)。
</Tip>
...
<Steps>
  ...
  <Step title="显示对话选择器">
    ...
    这显示一个交互式对话选择器，具有清洁的列表视图，显示：

    * 会话摘要（或初始提示）
    * 元数据：经过时间、消息计数和 git 分支
...
<Tip>
  ...
  * 每个 worktree 都有自己独立的文件状态，使其非常适合并行 Claude Code 会话
...
有关更多详情，请参阅[斜杠命令](/zh-CN/docs/claude-code/slash-commands)参考页面。
