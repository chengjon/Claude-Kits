...
* **计划模式和编辑**：在接受 Claude 的计划之前查看和编辑它们
* **自动接受编辑模式**：在 Claude 进行更改时自动应用它们
...
1. 单击编辑器侧边栏中的 Spark 图标打开 Claude Code 面板
2. 以与在终端中相同的方式提示 Claude Code
3. 观看 Claude 分析您的代码并建议更改
4. 直接在界面中查看和接受编辑
   * **提示**：将侧边栏拖宽以查看内联差异，然后单击它们以展开查看完整详情

### 使用第三方提供商（Vertex 和 Bedrock）

VS Code 扩展支持使用 Claude Code 与第三方提供商（如 Amazon Bedrock 和 Google Vertex AI）。当使用这些提供商配置时，扩展不会提示登录。要使用第三方提供商，请在 VS Code 扩展设置中配置环境变量：
...
| `ANTHROPIC_MODEL`             | 覆盖主模型                  | 覆盖模型 ID      | `"us.anthropic.claude-3-7-sonnet-20250219-v1:0"` |
...
* [Claude Code on Amazon Bedrock](/zh-CN/docs/claude-code/amazon-bedrock)
* [Claude Code on Google Vertex AI](/zh-CN/docs/claude-code/google-vertex-ai)

### 尚未实现

VS Code 扩展中尚未提供以下功能：

* **完整的 MCP 服务器配置**：您需要 [通过 CLI 配置 MCP 服务器](/zh-CN/docs/claude-code/mcp)，然后扩展将使用它们
* **子代理配置**：[通过 CLI 配置子代理](/zh-CN/docs/claude-code/sub-agents) 以在 VS Code 中使用它们
* **检查点**：在特定点保存和恢复对话状态
* **高级快捷键**：
  * `#` 快捷键添加到内存
  * `!` 快捷键直接运行 bash 命令
...
当 Claude Code 在启用自动编辑权限的 VS Code 中运行时，它可能能够修改 IDE 配置文件，这些文件可能会被您的 IDE 自动执行。这可能会增加在自动编辑模式下运行 Claude Code 的风险，并允许绕过 Claude Code 对 bash 执行的权限提示。

在 VS Code 中运行时，请考虑：

* 为不受信任的工作区启用 [VS Code 受限模式](https://code.visualstudio.com/docs/editor/workspace-trust#_restricted-mode)
* 对编辑使用手动批准模式
* 格外小心以确保 Claude 仅与受信任的提示一起使用
...
* 如果命令未安装：
  ...
  2. 搜索"Shell Command: Install 'code' command in PATH"（或您的 IDE 的等效选项）
